...
const carHeader: THeadProps<Car>[] = [
  ...
  },
  {
    ...
    title: 'Loja',
    render: (data) =>
      data.store ? <Chip canRemove={false}>{data.store?.name}</Chip> : <></>,
  },
  {
    key: 'created_at',
...
export default function ListVehiclesPage({
  ...
  const handleDelete = (car: Car) =>
    openDialog({
      ...
      onClose: (data) => {
        if (!data) return;
        router.delete(route(SuperRoutes.CAR_DELETE, { id: car.id }), {
          preserveScroll: true,
        });
      },
    });
    ...
    <>
      <>
        <Head title="Carros">
          <Button
            onClick={() =>
              router.visit(route(SuperRoutes.CAR_CREATE_VIEW), {
                preserveScroll: true,
              })
            }
          >
        ...
        <Table
          ...
          onEdit={(item) =>
            router.visit(route(SuperRoutes.CAR_EDIT_VIEW, { id: item.id }), {
              preserveScroll: true,
            })
          }
        />
