  ...
  const handleDeleteMotorcycle = (motorcycle: Motorcycle) =>
    openDialog({
      ...
      onClose: (data) => {
        if (!data) return;
        router.delete(
          route(SuperRoutes.MOTORCYCLE_DELETE, { id: motorcycle.id }),
          {
            preserveScroll: true,
          },
        );
  ...
  return (
    <>
      <Head title="Motos">
        <Button
          onClick={() =>
            router.visit(route(SuperRoutes.MOTORCYCLE_CREATE_VIEW), {
              preserveScroll: true,
            })
          }
        >
      ...
      <Table
        ...
        onEdit={(item) =>
          router.visit(
            route(SuperRoutes.MOTORCYCLE_EDIT_VIEW, { id: item.id }),
            {
              preserveScroll: true,
            },
          )
